@forelse ($latest_stores as $store_division)
<div class="col-lg-3">
        ...
        <div class="card-body p-0 rounded-2">
            ...
            <div class="main-store-banner">
                <img class="img-fluid rounded-2"
                    src="{{ !empty(optional($store_division->brand)->image) ? url('storage/' . optional($store_division->brand)->image) : asset('images/no-image(random).png') }}"
            ...
            <div class="px-3 pb-3">
                <div class="d-flex justify-content-between align-items-center">
                    <div class="store-logo">
                        <img class="img-fluid"
                            src="{{ !empty(optional($store_division->brand)->logo) ? url('storage/' . optional($store_division->brand)->logo) : asset('images/no-image(random).png') }}"
                            alt="arong-logo.png" />
                    </div>
                    <div class="store-rating">
                        <a href="{{ route('vendor.stores', optional($store_division->brand)->slug) }}"
                ...
                <div class="pt-4 d-flex justify-content-between store_title">
                    <a href="{{ route('vendor.stores', optional($store_division->brand)->slug) }}">
                        <div>
                            <h5 style="width: 98%;margin-bottom: 0.75rem;">{{ $store_division->title }}</h5>
                            <p>{{ $store_division->headquarter }}</p>
                        </div>
                    </a>
                    <div>
                ...
                {{-- <div class="bg-light mt-3 d-flex justify-content-between mb-3 p-2 rounded-3">
                    ...
                    <small class="text-sm">
                        Get UpTo <span class="main-color">{{ $store_division->badge }}</span> Off
                    </small>
                </div> --}}
            </div>
        </div>
