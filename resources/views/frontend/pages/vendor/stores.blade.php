                    ...
                    <ul class="nav nav-tabs border-0" id="myTab" role="tablist">
                        @foreach (optional($brand)->stores as $store)
                            <li class="nav-item w-100 vendor-store-items mb-2" role="presentation">
                                <button class="nav-link w-100 vendor-store {{ $loop->first ? 'active' : '' }} border-0 text-start" id="home-{{ optional($store)->id }}-tab" data-bs-toggle="tab"
                                    data-bs-target="#home-{{ optional($store)->id }}" type="button" role="tab" aria-controls="home-{{ optional($store)->id }}"
                                    aria-selected="true">
                                    <div class="card border-0 bg-transparent">
                                        <div class="card-body p-2 px-3">
                                            <h6 style="text-decoration: underline;" class="text-primary">{{ optional($store)->title }}</h6>
                                            <p class="pt-2">
                ...
                <div class="col-lg-7">
                    @foreach (optional($brand)->stores as $store)
                        <div class="tab-content">
                            <div class="tab-pane {{ $loop->first ? 'active' : '' }}" id="home-{{ optional($store)->id }}" role="tabpanel" aria-labelledby="home-{{ optional($store)->id }}-tab">
                                <div class="card rounded-0 shadow-sm border">
                                    <div class="card-body p-2">
                                        <iframe
                                            src="{{ optional($store)->url }}"
