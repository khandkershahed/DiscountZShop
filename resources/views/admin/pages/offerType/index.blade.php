    ...
    <div class="card card-flash">
        <div class="card-header mt-6">
            ...
            <div class="card-toolbar">
                ...
                @if (Auth::guard('admin')->user()->can('add.offertype'))
        ...
        <div class="card-body pt-0">
            <table id="kt_datatable_example_5" class="table table-striped table-row-bordered gy-5 gs-7 border rounded">
                <thead class="bg-dark text-light">
                    <tr class="text-center text-gray-400 fw-bolder fs-7 text-uppercase gs-0">
                        <th style="width: 10%" class="text-start">Sl</th>
                        <th style="width: 50%" class="text-start">Offer Type Name</th>
                        <th>Status</th>
                        <th>Action</th>
                    </tr>
                </thead>
                <tbody class="fw-bold text-gray-600">
                    ...
                    @foreach ($offer_types as $offer_type)
                        <tr class="text-center text-gray-400 fw-bolder fs-7 text-uppercase gs-0">
                            <td style="width: 10%" class="text-start">{{ $loop->iteration }}</td>
                            <td style="width: 50%" class="text-start">{{ $offer_type->name }}</td>
                            <td>
                                <span class="badge {{ $offer_type->status == 'active' ? 'bg-success' : 'bg-danger' }}">
                                    {{ $offer_type->status == 'active' ? 'Active' : 'InActive' }}
                            </td>
                            <td>
                                ...
                                @if (Auth::guard('admin')->user()->can('edit.offertype'))
                                    <a href="#" class="" data-bs-toggle="modal"
                                        data-bs-target="#EditModal_{{ $offer_type->id }}">
                                        <i class="fa-solid fa-pen text-primary"></i>
                                    </a>
                                @endif

                                @if (Auth::guard('admin')->user()->can('delete.offertype'))
                                    <a href="{{ route('admin.offer-type.destroy', $offer_type->id) }}"
                                        class="ms-2 delete text-danger" data-kt-docs-table-filter="delete_row">
