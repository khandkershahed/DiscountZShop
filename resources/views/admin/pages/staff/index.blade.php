                        ...
                        <tr>
                            ...
                            <td class="">
                                <img src="{{ !empty($staff->photo) ? url('storage/' . $staff->photo) : 'https://ui-avatars.com/api/?name=' . urlencode($staff->name) }}"
                                    height="40" width="40" alt="{{ $staff->name }}">
                            ...
                            <td class="text-start">
                                <label class="switch">
                                    <input type="checkbox" class="status-toggle" data-id="{{ $staff->id }}"
                            ...
                            <td>
                                ...
                                <a href="{{ route('admin.staff.destroy', $staff->id) }}" class="delete">
        ...
        <script>
            $(document).ready(function() {
                $('.status-toggle').change(function() {
                    ...
                    $.ajax({
                        url: '/admin/staff/status/' + adminId,
                        method: 'PUT',
                        data: {
                            _token: '{{ csrf_token() }}',
                            status: newStatus
                        ...
                        success: function(response) {
                            if (newStatus === 'active') {
                                toastr.success('Staff has been activated successfully.');
                            } else {
                                toastr.error('Staff has been deactivated successfully.');
                            }
                        },
                        error: function(xhr) {
